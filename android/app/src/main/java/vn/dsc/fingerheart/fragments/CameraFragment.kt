package vn.dsc.fingerheart.fragments
...
import vn.dsc.fingerheart.analyzer.LuminosityAnalyzer
import vn.dsc.fingerheart.databinding.FragmentCameraBinding
import vn.dsc.fingerheart.enums.CameraTimer
import vn.dsc.fingerheart.utils.*
...
class CameraFragment : BaseFragment<FragmentCameraBinding>(R.layout.fragment_camera) {
    ...
    // Controls the usage of AI model
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        initViews()
        context?.let { initTFLite(it.getAssets()) }
            ...
            imageAnalyzer!!.setAnalyzer(cameraExecutor, ImageAnalysis.Analyzer { imageProxy ->
                if (!isInference){
                    ...
                    Thread(Runnable {
                        ...
                        imgBitmap = rotateBitmap(imgBitmap, rotationDegrees.toFloat())
                        var isFingerHeartFound = false
                        if (imgBitmap != null) {
                            isFingerHeartFound = isHaveFingerHeart(imgBitmap)
                            Log.e("TAG", "FingerHeart Found: " + isFingerHeartFound)
                        } else {
                            Log.e("TAG", "imgBitmap is null")
                        }
                        if (isFingerHeartFound){
                            binding.btnTakePicture.callOnClick()
                            when (selectedTimer) {
                                CameraTimer.S3 -> { Thread.sleep(4000) }
                                CameraTimer.S10 -> { Thread.sleep(11000) }
                                else -> {}
                            }
                        }
                        isInference = false
    ...
    companion object {
        private const val TAG = "FingerHeart"
    ...
    /**
     * A native method that is implemented by the 'fingerheart' native library,
     * which is packaged with this application.
     */
    external fun isHaveFingerHeart(Bitmap: Bitmap): Boolean
    external fun initTFLite(assetManager: AssetManager): Boolean
}
